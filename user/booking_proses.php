...
if ($id_paket === 0 && !empty($nama_paket)) {
    ...
    if (!$row) {
        $like = "%" . $nama_paket . "%";
        $stmt = $koneksi->prepare("SELECT * FROM paket WHERE nama_paket LIKE ? LIMIT 1");
    ...
    if (!$row && $harga > 0) {
        $stmt = $koneksi->prepare("SELECT * FROM paket WHERE harga = ? LIMIT 1");
    ...
    $stmt = $koneksi->prepare("SELECT * FROM paket WHERE id_paket = ?");
